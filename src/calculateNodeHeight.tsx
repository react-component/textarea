...
const UNIQUE_KEY = `rc-textarea-measure-${String(Math.random()).replace(
  '.',
  '',
)}`;
...
export default function calculateAutoSizeStyle(
  ...
  uiTextNode.setAttribute(UNIQUE_KEY, UNIQUE_KEY);
  updateCSS(
    `
...
`,
  ...
  // uiTextNode.removeAttribute(UNIQUE_KEY);

  let height = uiTextNode.scrollHeight;
  console.log('scrollHeight:', height);

  const textareaStyle = getComputedStyle(uiTextNode);
  const { boxSizing } = textareaStyle;

  if (boxSizing === 'border-box') {
    // border-box: add border, since height = content + padding + border
    const { borderTopWidth, borderBottomWidth } = textareaStyle;
    height += parseFloat(borderTopWidth) + parseFloat(borderBottomWidth);
  } else if (boxSizing === 'content-box') {
    // remove padding, since height = content
    const { paddingTop, paddingBottom } = textareaStyle;
    height -= parseFloat(paddingTop) + parseFloat(paddingBottom);
  }
  console.log('Final:', height);

  removeCSS(UNIQUE_KEY);
